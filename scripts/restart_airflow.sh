...
export airflow_instance_public_ip=$(aws ec2 describe-instances --filters 'Name=tag:Name,Values=de-c4w4a2-da-instance' --query "Reservations[].Instances[].PublicIpAddress" --output text)
...
export airflow_instance_id=$(aws ec2 describe-instances --filters 'Name=tag:Name,Values=de-c4w4a2-da-instance' --query "Reservations[].Instances[].InstanceId" --output text)
